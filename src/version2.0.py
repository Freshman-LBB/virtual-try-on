...
parser.add_argument('--openpose_proto', default='models/openpose_pose_coco.prototxt', help='Path to OpenPose .prototxt model was trained on COCO dataset.')
parser.add_argument('--openpose_model', default='models/openpose_pose_coco.caffemodel', help='Path to OpenPose .caffemodel model was trained on COCO dataset.')
...
class BilinearFilter(object):
    ...
    def _precompute_coeffs(self, inSize, outSize):
        ...
        points = np.array([np.arange(row) + xmins[i] for i, row in enumerate(bounds[1::2])], dtype=object) / filterscale
...
class CpVton(object):
    ...
    def get_warped_cloth(self, cloth_img, agnostic, height=256, width=192):
        # 1. 读取特征点数据
        pair_data = self.load_pair_points(height, width)

        if pair_data is None:
            # 如果点数据读取失败，返回原始cloth
            print("特征点读取失败，使用原始图像")
            return cv.dnn.blobFromImage(
                cv.resize(cloth_img, (width, height)),
                1.0 / 127.5,
                (width, height),
        ...
        # cloth_points: 原始衣服特征点
        # human_points: 人体特征点
        cloth_points = pair_data['cloth_points']
        human_points = pair_data['human_points']

        # 额外的数据验证
        if not cloth_points or not human_points:
            print("特征点为空，使用原始图像")
            return cv.dnn.blobFromImage(
                cv.resize(cloth_img, (width, height)),
                1.0 / 127.5,
        ...
        # 2. 创建TPS变形器
        tps = self.create_thin_plate_spline(
            cloth_points,
            human_points
        )
        ...
        try:
            ...
            for pair_data in pairs_data:
                ...
                if pair_data.get('pair') == current_pair:
                    # 使用 cloth_points 作为源特征点
                    # 使用 pair_points 作为目标特征点
                    cloth_points = pair_data.get('cloth_points', [])
                    pair_points = pair_data.get('pair_points', [])

                    # 缩放点坐标
                    cloth_points = self.scale_points(
                        cloth_points,
                        height,
                        width
                    )
                    pair_points = self.scale_points(
                        pair_points,
                        height,
                        width
                    )
                    ...
                    print(f"cloth_points: {cloth_points}")
                    print(f"pair_points: {pair_points}")

                    if len(cloth_points) != 12 or len(pair_points) != 12:
                        print(f"警告：点数不匹配 {current_pair}")

                    return {
                        'cloth_points': cloth_points,
                        'human_points': pair_points  # 使用 pair_points 作为目标点
                    }
    ...
    def create_thin_plate_spline(self, src_points, dst_points):
        """
        创建薄板样条插值变形器
        增加更严格的数据验证
        """
        try:
            ...
            if src_points.ndim != 2 or dst_points.ndim != 2:
                raise ValueError("特征点必须是二维数组")

            # 确保点数相同且大于0
            if len(src_points) == 0 or len(src_points) != len(dst_points):
                raise ValueError(f"特征点数量不匹配 src: {len(src_points)}, dst: {len(dst_points)}")

            # 确保每个点有2个坐标
            if src_points.shape[1] != 2 or dst_points.shape[1] != 2:
                raise ValueError("每个点必须有2个坐标")

            # 调试：打印特征点
            print("源特征点:", src_points)
            print("目标特征点:", dst_points)

            # 创建TPS变形器
            tps = cv.createThinPlateSplineShapeTransformer()

            # 创建匹配
            matches = [cv.DMatch(i, i, 0) for i in range(len(src_points))]

            # 估算变形
            tps.estimateTransformation(
                np.array([src_points], dtype=np.float32),  # 增加一个维度
                np.array([dst_points], dtype=np.float32),  # 增加一个维度
                matches
            )

            return tps
        except Exception as e:
            print(f"TPS变形器创建失败: {e}")
            # 如果失败，可以返回None或抛出异常
            return None

    def apply_tps_transform(self, cloth_img, tps, height, width):
        """
        应用TPS变形
        增加更多错误处理
        """
        try:
            ...
            # 调整cloth_img大小
            resized_cloth = cv.resize(cloth_img, (width, height))

            # 应用变形
            warped_cloth = tps.warpImage(
                resized_cloth,
                borderMode=cv.BORDER_TRANSPARENT
            )

            # 转换为深度学习所需格式
            cloth = cv.dnn.blobFromImage(
                warped_cloth,
                1.0 / 127.5,
                (width, height),
                mean=(127.5, 127.5, 127.5),
...
if __name__ == "__main__":
    try:
        ...
        model_files = [
            ...
            ('openpose_proto', findFile(args.openpose_proto)),
            ('openpose_model', findFile(args.openpose_model))
        ]
        ...
        # 在原始图像上标注点
        person_img_with_points = draw_points(person_img, pair_points_scaled, color=(0, 0, 255), radius=8)  # 蓝色
        cloth_img_with_points = draw_points(cloth_img, cloth_points_scaled, color=(255, 0, 0), radius=8)  # 红色

        # 在合成图像上标注点
        output_with_points = draw_points(output, pair_points_scaled, color=(0, 0, 255), radius=5)  # 人体点 蓝色
        output_with_points = draw_points(output_with_points, cloth_points_scaled, color=(255, 0, 0), radius=5)  # 衣服点 红色
